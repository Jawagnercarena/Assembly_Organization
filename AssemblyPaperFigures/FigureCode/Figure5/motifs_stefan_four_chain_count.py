...
np.save('pyc_weight_matrix_668.npy', weight_matrix)
...
# print(type(three_chain_results[0]))
...
chain_membership_counts_by_assembly_by_cell['All Assemblies'] = {}
...
for chain in tqdm(four_chain_results):
    origin_cell_in_pyc_id, second_cell_in_pyc_id, next_to_last_cell_in_pyc_id, terminal_cell_in_pyc_id = chain['A'], chain['B'], chain['C'], chain['D']
                    ...
                    for temp_terminal_assembly_id in terminal_assembly_ids:
                        if temp_origin_assembly_id == temp_terminal_assembly_id: # If they're both in the same assembly
                            ...
                            if terminal_cell_in_root_id in chain_membership_counts_by_assembly_by_cell[f'Assembly {temp_origin_assembly_id+1}'].keys():
                                ...
                                chain_membership_counts_by_assembly_by_cell[f'Assembly {temp_origin_assembly_id+1}'][terminal_cell_in_root_id] = 1
                            # Also add them to the all assemblies pool
                            if origin_cell_in_root_id in chain_membership_counts_by_assembly_by_cell['All Assemblies'].keys():
                                chain_membership_counts_by_assembly_by_cell['All Assemblies'][origin_cell_in_root_id] += 1
                            else:
                                chain_membership_counts_by_assembly_by_cell['All Assemblies'][origin_cell_in_root_id] = 1
                            if terminal_cell_in_root_id in chain_membership_counts_by_assembly_by_cell['All Assemblies'].keys():
                                chain_membership_counts_by_assembly_by_cell['All Assemblies'][terminal_cell_in_root_id] += 1
                            else:
                                chain_membership_counts_by_assembly_by_cell['All Assemblies'][terminal_cell_in_root_id] = 1
...
print(chain_membership_counts_by_assembly_by_cell)
...
assembly_list = [f'Assembly {assembly_id+1}' for assembly_id in list(functional_cell_indexes_by_assembly.keys())]
assembly_list.append('All Assemblies')

for assembly_id in assembly_list:
    participation_count_list = []
    for cell_id in chain_membership_counts_by_assembly_by_cell[assembly_id].keys():
        participation_count_list.append(chain_membership_counts_by_assembly_by_cell[assembly_id][cell_id]) 
    print(f'Assembly: {assembly_id}')
    print(f'\tMean Participation: {np.mean(participation_count_list)}')
    print(f'\tVersus Non-Assembly Mean Participation: {np.mean(non_assembly_participation_count_list)}')
    ...
    print(f'Ranksum statistic: {stat}, p={p_value}')

    plt.figure()
    plt.title(f'{assembly_id} Chain Membership Counts\np={p_value:.4f}')
    ...
    plt.legend([f'{assembly_id} Cells', 'Cells Not in Assemblies'])
    plt.savefig(f'Chain Length Four Participation Assembly {assembly_id}.png')
...
# for chain in tqdm(four_chain_results):
#     origin_cell_in_pyc_id, second_cell_in_pyc_id, middle_cell_two_in_pyc_id, terminal_cell_in_pyc_id = chain['A'], chain['B'], chain['C'], chain['D']
...
#         psd_sizes_origin_to_mid = list(synapse_table.query('pre_pt_root_id == @origin_cell_in_root_id and post_pt_root_id == @second_cell_in_root_id')['size'].values)
#         psd_sizes_mid_to_second_mid = list(synapse_table.query('pre_pt_root_id == @second_cell_in_root_id and post_pt_root_id == @middle_cell_two_in_root_id')['size'].values)
